...
function b(){ const fU=new n("New_1");const fV=(fU.h.push(new H("status", c().View)),fU);const fW=(fV.h.push(q(fV.k, "setupdrm", () => fV.i, () => { const f1=null;P(M(() => N(s(d()), () => O(null))), null);})),fV);const fX=r(fW.k, fW.h, []);const fY=new i(fX[1], F(fX[0]));let fZ=(fW.i=fY,fY);const f0=fZ.Doc;u0031("");bi.RunById("main", f0);}
function c(){ return I.statusMessage;}
function d(){ const fU=null;return new Promise((fV) => { const fW=globalThis.navigator;let fX=(globalThis.document.getElementById("status"),new Promise((fY) => { let fZ;let f0;fY(fW.requestMediaKeySystemAccess("com.widevine.alpha", [(fZ={},fZ.initDataTypes=["cenc"],fZ.videoCapabilities=[(f0={},f0.contentType="video/mp4",f0)],fZ)]).then((f1) => { c().Set("\u2705 DRM supported: "+String(f1.keySystem));return Promise.resolve(null);}));})["catch"]((fY) => { const fZ=t(fY);c().Set("\u274c DRM not supported: "+String(fZ.message));return Promise.resolve(null);}));fV(fX);});}
...
let I=fO((fU) => class fV { static { I=fU(this);} static statusMessage;static { this.statusMessage=bk.Create_1("Waiting for DRM check...");} });
